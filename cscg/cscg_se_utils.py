...
def prepare_obs_act(
  ...
  if not training:
    if observations.shape != (len(observations), num_emissions):
      observations = jax.nn.one_hot(observations, num_emissions)
  ...
  observations = observations.reshape(
      local_device_count, per_device_seq_len, -1
  )
